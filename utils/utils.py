...
def visualize_point_cloud(points, label_function, confidence_threshold):
    # Convert label function to colors for visualization
    colors = np.zeros((len(points), 3))
    for i in range(len(points)):
        max_confidence = label_function[i].max()
        if max_confidence < confidence_threshold:
            colors[i] = [0, 0, 0]  # Black color for low confidence points
        else:
...
# 定义一个函数，用于在给定标签集中为某些类别随机选取特定数量的样本
...
def calculate_iou(predicted, y_all, n_classes):
    # Initialize arrays to hold TP, FP, FN, and TN counts for each class
